    ...
    <div className={cn("fixed bottom-4 sm:top-6 left-1/2 -translate-x-1/2 z-50 pointer-events-none", className)}>
      <div className="flex items-center gap-2 bg-black/5 border border-white/10 backdrop-blur-lg p-1 rounded-full shadow-lg pointer-events-auto">
        {items.map((item) => {
          ...
          return (
            <a key={item.name} href={item.url} onClick={(e) => { e.preventDefault(); setActiveTab(item.name); onNavItemClick(item.id); }} className={cn("relative cursor-pointer text-sm font-medium px-4 py-2 rounded-full transition-colors", "text-white/80 hover:text-white", isActive && "text-white")}>
              <div className="relative z-20 flex items-center gap-2">
                <Icon size={18} strokeWidth={2} />
                <span className="hidden md:inline">{item.name}</span>
              </div>
              {isActive && (
                <motion.div layoutId="lamp" className="absolute inset-0 w-full bg-white/10 rounded-full" initial={false} transition={{ type: "spring", stiffness: 300, damping: 30 }}>
